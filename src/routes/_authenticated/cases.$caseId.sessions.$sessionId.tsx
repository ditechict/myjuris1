...
function SessionPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: s, error: se }, { data: c }] = await Promise.all([
        ...
        supabase
          .from("cases")
          .select("id,case_name,suit_number,plaintiff,defendant")
          .eq("id", caseId)
          .maybeSingle(),
      ]);
  ...
  const startRec = async () => {
    ...
    if (sr.supported) {
      try {
        sr.start(() => durationRef.current * 1000);
      } catch {
        /* surfaced via sr.error */
      }
    }
    try {
      ...
      try {
        ...
        const { error: updErr } = await supabase
          ...
          .update({
            ...
            bookmarks: bookmarks as unknown as never,
  ...
  const addBookmark = () => {
    ...
    setBookmarks((p) => [
      ...p,
      { id: uid(), label, timeMs: durationRef.current * 1000, createdAt: new Date().toISOString() },
    ]);
    setBookmarkLabel("");
  ...
  const exportDocx = async () => {
    ...
    const blob = await exportTranscriptDocx({
      ...
      suitNumber: caseRow.suit_number,
  ...
  const exportAudio = async () => {
    ...
    if (audioUrl) {
      ...
      a.href = audioUrl;
      a.download = `${caseRow?.suit_number ?? "session"}.audio`;
          ...
          <Card className="p-5">
            <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
              <div className="flex items-center gap-3">
                <div
                  className={`size-3 rounded-full ${isRecording ? "bg-destructive recording-pulse" : isPaused ? "bg-warning" : "bg-muted-foreground/40"}`}
                />
                <div>
                  ...
                  <div className="text-2xl font-mono tabular-nums tracking-tight">
                    {formatTime(recorder.durationSeconds || (session?.duration_seconds ?? 0))}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {recordingState === "idle" || recordingState === "stopped" ? (
                  <Button onClick={startRec} disabled={persisting}>
                    <Mic className="size-4" /> Record
                  </Button>
                ) : null}
                {isRecording && (
                  <Button onClick={pauseRec} variant="secondary">
            ...
            {!sr.supported && (
              <div className="mt-3 flex items-start gap-2 text-xs text-muted-foreground">
                <AlertCircle className="size-4 mt-0.5 shrink-0" />
                <span>
                  Live transcription requires a Chromium‑based browser (Chrome, Edge). Audio
          ...
          <Card className="p-4">
            ...
            <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2 text-xs">
              <DiagRow
                ...
                ok={isSecure}
                value={isSecure ? "yes" : "no — required"}
              />
              <DiagRow
            ...
            <div className="grid sm:grid-cols-[1fr_2fr_auto] gap-3 items-end">
              <div className="space-y-1.5">
                <label className="text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-1.5">
                  <UserCircle className="size-3.5" /> Active speaker
                </label>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="e.g. Objection overruled"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                      addBookmark();
              ...
              <Button onClick={addBookmark} variant="secondary">
          ...
          <Card className="p-0">
              ...
              <div className="flex items-center gap-2">
                {savedAt && (
                  <span className="text-[11px] text-muted-foreground hidden sm:inline">
                    Local cache · {new Date(savedAt).toLocaleTimeString()}
                  </span>
                )}
                <Button
                  size="sm"
                  ...
                  disabled={
                    diarizing || recordingState === "recording" || recordingState === "paused"
                  }
                  title="Run AI speaker diarization on the recorded audio"
                >
                  {diarizing ? (
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={exportDocx}>
                      <FileText className="size-4" /> Transcript (.docx)
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={exportAudio}>
                      <Mic className="size-4" /> Audio file
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={async () => {
            ...
            <ScrollArea className="h-[460px]">
              <div className="p-4 space-y-3">
                {transcript.length === 0 && !interimDisplay && (
                  <div className="text-sm text-muted-foreground text-center py-12">
                    {isRecording
                      ...
                      : "Press Record to begin capturing audio and live transcription."}
                  </div>
                )}
                {transcript.map((seg) => (
                ...
                {interimDisplay && (
                  <div>
                    <div className="text-[11px] font-mono text-muted-foreground tabular-nums mb-0.5">
        ...
        <div className="space-y-4">
            ...
            <div className="p-3 max-h-[420px] overflow-auto">
              {sortedBookmarks.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">
                  No flags yet. Drop one with the Flag button.
                </p>
              ) : (
                <ul className="space-y-2">
                  {sortedBookmarks.map((b) => (
                    <li
                      key={b.id}
                      className="flex items-start gap-3 p-2.5 rounded-md bg-muted/40 border border-border"
                    >
                      <span className="mt-0.5 text-[11px] font-mono tabular-nums text-warning shrink-0">
                        {formatTime(b.timeMs / 1000)}
                      </span>
                      <span className="text-sm flex-1 break-words">{b.label}</span>
                      <button
                        onClick={() => setBookmarks((p) => p.filter((x) => x.id !== b.id))}
          ...
          <Card className="p-4">
            <h3 className="text-sm font-medium mb-3 flex items-center gap-2">
              <CheckCircle2 className="size-4 text-success" /> Status
            </h3>
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li className="flex items-center justify-between">
                <span>Microphone</span>
                <Badge variant={recorder.error ? "destructive" : "outline"}>
                  {recorder.error ? "Blocked" : "Ready"}
                </Badge>
              </li>
              <li className="flex items-center justify-between">
                <span>Live transcription</span>
                <Badge variant={sr.supported ? "outline" : "secondary"}>
                  {sr.supported ? (sr.active ? "Active" : "Idle") : "Unsupported"}
                </Badge>
              </li>
              <li className="flex items-center justify-between">
                <span>Local auto‑save</span>
                <Badge variant="outline">{savedAt ? "Active" : "Standby"}</Badge>
              </li>
              <li className="flex items-center justify-between">
                <span>Cloud sync</span>
                <Badge variant="outline">{persisting ? "Saving…" : "Synced"}</Badge>
              </li>
            </ul>
          </Card>
        </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            myJuris needs microphone access to record audio and generate a transcript. Your browser
            has blocked or denied the request.
          </DialogDescription>
        </DialogHeader>
        <div className="text-sm">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Recording requires a secure (HTTPS) page. Make sure no other app is currently using the
            microphone.
          </p>
        </div>
        <DialogFooter>
